...
function ProfileContent() {
  ...
  const initialTab = searchParams.get("tab") as "certifications" | "followers" | "followings" | "badges" | "streaks" | null; // ← streaks 추가

  const isAuthenticated = useAuthStore((state) => state.isAuthenticated);
  const { data: user, isLoading, isError, error } = useUserProfile();

  const [activeTab, setActiveTab] = useState<"certifications" | "followers" | "followings" | "badges" | "streaks">( // ← streaks 추가
      (initialTab && ["certifications", "followers", "followings", "badges", "streaks"].includes(initialTab)) // ← streaks 추가
          ? initialTab
          : "certifications"
  );
          ...
          {activeTab === "followings" && (
              <div className="grid grid-cols-1 gap-6">
                {isLoadingFollowings ? (
                    ...
                    <>
                      <div className="text-sm text-gray-900 font-medium">
                        이 <span className="font-bold text-blue-600">{followings?.length}</span>명을 팔로우 중
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
