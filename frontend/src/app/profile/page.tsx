...
function ProfileContent() {
  ...
  const [activeTab, setActiveTab] = useState<
    "certifications" | "followers" | "followings" | "badges" | "streaks"
  >( // ← streaks 추가
    initialTab &&
      ["certifications", "followers", "followings", "badges", "streaks"].includes(initialTab) // ← streaks 추가
      ? initialTab
      : "certifications"
  );
            ...
            {isLoadingFollowers ? (
              ...
              <>
                <div className="text-sm text-gray-900 font-medium">
                  이 <span className="font-bold text-blue-600">{followers?.length}</span>명의 팔로워
                </div>
                    ...
                    >
                      <div className="p-4">
                        <div className="flex items-start justify-between mb-3">
                          ...
                          >
                            <p className="font-bold text-lg text-gray-900 group-hover:text-blue-600 transition-colors line-clamp-1">
                              {follower.nickname}
                            </p>
                        ...
                        <div className="flex gap-2 pt-2">
                          ...
                          <div className="flex-1">
                            <FollowButton
                              ...
                              size="sm"
                              initialIsFollowing={
                                followings?.some((f) => f.loginId === follower.loginId) ?? false
                              }
                            />
                          </div>
                        </div>
              ...
              <div className="text-center py-12 text-gray-500">팔로워가 없습니다.</div>
            )}
          </div>
        )}
        ...
        {activeTab === "followings" && (
          <div className="grid grid-cols-1 gap-6">
            {isLoadingFollowings ? (
              ...
              <>
                <div className="text-sm text-gray-900 font-medium">
                  이 <span className="font-bold text-blue-600">{followings?.length}</span>명을
                  팔로우 중
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div className="text-center py-12 text-gray-500">팔로잉이 없습니다.</div>
            )}
          </div>
        )}
      </main>
