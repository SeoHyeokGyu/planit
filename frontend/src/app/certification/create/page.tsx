...
function CreateCertificationContent() {
  ...
  const { file, isCompressing, error: imageError, handleFileChange } = useImageUpload();
      ...
      if (createResponse.success && createResponse.data) {
        ...
        if (file) {
          ...
          if (!uploadResponse.success) {
            ...
            setFormError(
              uploadResponse.message || "사진 업로드에 실패하여 인증 생성이 취소되었습니다."
            );
            return;
          }
        }
      ...
      <div className={layoutStyles.containerMd}>
        ...
        <div className={headerStyles.wrapper}>
          <div className={headerStyles.content}>
            <div className={`${headerStyles.icon} ${themeStyles.success.bg}`}>
              <CheckCircle className="w-6 h-6" />
            </div>
            <h1 className={`${headerStyles.title} ${themeStyles.success.text}`}>챌린지 인증하기</h1>
          </div>
          <p className={headerStyles.description}>오늘의 노력을 기록하고 공유해보세요</p>
        </div>

        <Card className={cardStyles.base}>
          <CardHeader className={`${cardStyles.headerGradient} ${themeStyles.success.headerBg}`}>
            <div className="flex items-center gap-3">
              <div
                className={`w-10 h-10 ${themeStyles.success.bg} rounded-lg flex items-center justify-center`}
              >
          ...
          <CardContent className="pt-6 bg-white">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <Button
                type="submit"
                className={`${buttonStyles.submit} ${themeStyles.success.btn}`}
