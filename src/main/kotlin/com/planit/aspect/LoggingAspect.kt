...
class LoggingAspect {
    ...
    @Pointcut("execution(* com.planit.controller..*(..))")
    fun controllerMethods() {}

    @Pointcut("execution(* com.planit.service..*(..))")
    fun serviceMethods() {}

    @Pointcut("execution(* com.planit.repository..*(..))")
    ...
    private fun executeLogging(
        ...
        try {
            ...
            // [응답] 로그
            logger.debug("[$uuid] [$layer 응답] <- $fullMethodName | 결과: $resultString | 처리시간: ${duration}ms")
            return result

        } catch (e: Exception) {
            val duration = System.currentTimeMillis() - startTime
            // [에러] 로그
            logger.error("[$uuid] [$layer 에러] <- $fullMethodName | ${e.javaClass.simpleName}: ${e.message} | 처리시간: ${duration}ms")
            throw e
        }
    }

    private fun getOrInitUuid(): String {
        return MDC.get(MDC_KEY) ?: run {
