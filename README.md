...
#### 1.1 랜딩 페이지
- **Hero 섹션**: 그라데이션 배경 + 프로젝트 로고 + CTA 버튼
- **기능 소개**: 6가지 핵심 기능 카드 (AI 추천, 실시간 피드, AI 코치, 게임화, 소셜, AI 검증)
- **통계 섹션**: 주요 수치 표시
- **개발자 도구**: API 테스트, Swagger UI 링크

#### 1.2 로그인/회원가입
- **로그인 폼**: 이메일, 비밀번호 입력
- **회원가입 폼**: 이메일, 비밀번호, 닉네임, 이메일 중복 검증

### 2. 대시보드

#### 2.1 메인 대시보드
- **통계 카드**: 참여 중인 챌린지, 총 인증 수, 팔로워, 팔로잉 수
- **포인트/배지/스트릭**: 현재 포인트, 획득 배지 수, 최고 스트릭
- **참여 중인 챌린지**: 진행 중인 챌린지 목록 (진행률 표시)
- **최근 활동 피드**: 최근 3개 인증 미리보기
- **AI 추천 챌린지**: 나에게 맞는 챌린지 추천

### 3. 챌린지

#### 3.1 챌린지 목록
- **필터링**: 카테고리, 난이도, 기간, 상태별 필터
- **정렬**: 최신순, 이름순, 난이도순, 인기순
- **검색**: Full-Text Search 키워드 검색
- **챌린지 카드**: 제목, 설명, 참여자 수, 카테고리, 난이도 표시
- **AI 추천 섹션**: Gemini AI 기반 추천 챌린지

#### 3.2 챌린지 상세
- **챌린지 정보**: 제목, 설명, 카테고리, 난이도, 기간
- **통계**: 참여자 수, 인증 수, 완료율, 조회수
- **참여 버튼**: 챌린지 참여/탈퇴
- **참여자 목록**: 현재 참여 중인 사용자 목록
- **인증 목록**: 챌린지 관련 인증 피드

#### 3.3 챌린지 생성/편집
- **기본 정보 폼**: 제목, 설명, 카테고리, 난이도, 기간 선택
- **AI 챌린지 생성기**: 자연어 입력으로 자동 생성 버튼
- **생성 결과 미리보기**: AI가 생성한 챌린지 정보 폼에 자동 채우기

### 4. 인증

#### 4.1 인증 작성
- **챌린지 선택**: 참여 중인 챌린지 드롭다운
- **제목/내용 입력**: 인증 제목, 내용 작성
- **사진 업로드**: 이미지 선택 및 미리보기
- **AI 분석 결과**: Gemini AI 적합성 판단 결과 표시

#### 4.2 인증 상세
- **인증 정보**: 제목, 내용, 작성자, 작성일
- **사진 표시**: 업로드된 인증 사진
- **AI 분석 결과**: 적합성 판단, 분석 결과 텍스트
- **좋아요/댓글**: 좋아요 토글, 댓글 목록/작성
- **재분석 버튼**: AI 인증 재분석 기능

#### 4.3 내 인증 목록
- **인증 카드**: 제목, 챌린지명, 작성일, 좋아요/댓글 수
- **페이징**: 무한 스크롤

### 5. 실시간 피드

#### 5.1 전체 피드
- **실시간 알림**: "OO님이 방금 'XX 챌린지'를 인증했습니다!" SSE 알림
- **인증 카드**: 사용자 프로필, 챌린지명, 인증 제목, 사진 썸네일
- **무한 스크롤**: React Query 기반 페이징
- **정렬 옵션**: 최신순, 좋아요순, 댓글순, 인기순

#### 5.2 팔로잉 피드
- **팔로우 사용자 인증**: 팔로우한 사용자의 인증만 표시
- **실시간 업데이트**: SSE 기반 실시간 알림

### 6. 랭킹

#### 6.1 리더보드
- **포디움 UI**: 상위 3명 강조 표시 (🥇🥈🥉)
- **순위 목록**: 순위, 사용자명, 포인트, 순위 변동 표시
- **기간 선택**: 주간 / 월간 / 전체 탭
- **실시간 업데이트**: SSE 기반 순위 변동 즉시 반영
- **내 순위**: 본인 순위 하이라이트
...
#### 7.1 내 프로필
- **프로필 헤더**: 닉네임, 이메일, 가입일
- **통계 요약**: 총 인증 수, 포인트, 팔로워/팔로잉 수
- **탭 네비게이션**: 인증, 팔로워, 팔로잉, 배지, 스트릭

#### 7.2 다른 사용자 프로필
- **팔로우 버튼**: 팔로우/언팔로우 토글
- **사용자 통계**: 해당 사용자의 인증 수, 배지, 스트릭
- **인증 목록**: 사용자의 공개 인증 목록

### 8. 게임화 요소

#### 8.1 포인트 통계 (Stats 페이지)
- **포인트 요약**: 총 포인트, 오늘 획득, 이번 주 획득
- **일별 포인트 차트**: Recharts 기반 막대 그래프
- **누적 포인트 차트**: 선 그래프로 추이 표시
- **포인트 히스토리**: 적립/차감 내역 목록

#### 8.2 배지 시스템
- **전체 배지 목록**: 획득/미획득 배지 표시
- **배지 카드**: 배지 이름, 설명, 등급, 획득 조건
- **획득 배지**: 획득한 배지 강조 표시
- **진행률**: 미획득 배지의 진행률 표시 (예: 50/100 인증)

#### 8.3 스트릭 캘린더
- **GitHub 스타일 잔디**: 일별 활동 기록 시각화
- **현재 스트릭**: 연속 인증 일수 표시
- **최고 스트릭**: 역대 최고 연속 인증 일수
- **월별 네비게이션**: 이전/다음 달 이동

### 9. 알림

#### 9.1 알림 드롭다운
- **실시간 알림**: SSE 기반 실시간 알림 수신
- **미읽음 카운트**: 상단 네비게이션에 빨간 배지 표시
- **알림 목록**: 최근 알림 10개 미리보기
