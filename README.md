...
- **게임화**: 포인트/레벨 시스템 (완료), 배지/스트릭 시스템 (계획 중)
...
### 11. 경험치/포인트/레벨 시스템 (Experience & Points & Level System) - 완료
- [x] 경험치(EXP) 규칙 정의 (인증 +15, 댓글 +5, 좋아요 +2, 배지 +25)
- [x] 포인트(Point) 규칙 정의 (인증 +10, 댓글 +2, 좋아요 +1, 배지 +5) - 상점용 재화
- [x] 경험치 적립 API (행동별 경험치 부여)
- [x] 포인트 적립 API (행동별 포인트 부여)
- [x] 레벨 계산 로직 (경험치 기반: 100 EXP = 1 레벨)
- [x] 레벨업 감지 및 알림 발송
- [x] 경험치 히스토리 API (페이징, 필터링)
- [x] 포인트 히스토리 API (페이징, 필터링)
- [x] 사용자 진행도 조회 API (현재 레벨, 경험치, 다음 레벨까지 필요 EXP, 포인트)
- [x] 포인트 차감 API (향후 상점 기능 대비)
- [x] 통계 API (일별 경험치, 포인트, 레벨 변화 - 날짜 범위 지정)
...
- `GET /api/points/experience/me` - 내 경험치/레벨 조회
- `GET /api/points/experience/me/history` - 내 경험치 히스토리 (페이징)
- `GET /api/points/experience/me/statistics` - 경험치 통계 (일별, 레벨 추적)
- `GET /api/points/me/progress` - 내 진행도 조회 (레벨, 경험치, 포인트)
...
| **구현된 API 엔드포인트** | 43+ |
| **구현된 프론트엔드 페이지** | 16+ |
| **백엔드 기능 섹션** | 11개 (인증, 사용자, 챌린지, 인증, 피드, 팔로우, 알림, 포인트/레벨, 배지, 모니터링, 스케줄링) |
| **구현된 기능** | 사용자 인증, 챌린지 관리, 인증 시스템, 팔로우, 실시간 피드, 알림, 포인트/경험치/레벨, 배지 시스템, 통계 |
| **완료율** | ~55% (주요 핵심 기능 완료) |
...
- Redis 캐싱 (조회수, 팔로우 수)
- LoggingAspect 개선 (Service, Repository 레이어 로깅 + MDC)
- 챌린지 알림 스케줄러 (자동 알림 푸시)
- 포인트/경험치/레벨 시스템 (적립, 차감, 레벨업 알림, 히스토리, 진행도 API)
- 포인트/경험치 통계 API (일별 데이터, 누적 추이, 레벨 변화 추적)
- 배지 시스템 (배지 정의, 조건 체크, 자동 부여, 알림, BadgeChecker 패턴)
