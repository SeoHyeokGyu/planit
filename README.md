...
- [ ] 경험치(EXP) 규칙 정의 (인증 +15, 댓글 +5, 좋아요 +2, 배지 +25)
- [ ] 포인트(Point) 규칙 정의 (인증 +10, 댓글 +2, 좋아요 +1, 배지 +5) - 상점용 재화
- [ ] 경험치 적립 API (행동별 경험치 부여)
- [ ] 포인트 적립 API (행동별 포인트 부여)
- [ ] 레벨 계산 로직 (경험치 기반: 100 EXP = 1 레벨)
- [ ] 레벨업 감지 및 알림 발송
- [ ] 경험치 히스토리 API (페이징, 필터링)
- [ ] 포인트 히스토리 API (페이징, 필터링)
- [ ] 사용자 진행도 조회 API (현재 레벨, 경험치, 다음 레벨까지 필요 EXP, 포인트)
